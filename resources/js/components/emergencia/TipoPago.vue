<template>
        <div class="body-miabogado">
            <h3>Informació/emergencian Importante</h3>
...
export default {
    ...
    methods:{
        tipoPago(){
            axios.post("/emergencia/request/tipo/pago",{numeroConsulta:this.numeroConsulta,modoPago:this.modoPago})
            .then((result) => {
             if(this.modoPago === "tarjeta")  {
                window.location.href = "/emergencia/pago/tarjeta/"+this.codigo
             }else if(this.modoPago === "deposito"){
                window.location.href = "/emergencia/cuentas/deposito/"+this.codigo
             }
            }).catch((err) => {
