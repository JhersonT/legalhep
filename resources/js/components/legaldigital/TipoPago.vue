<template>
        ...
        <div class="body-miabogado" >
            ...
            <h3 class="titulo-consulta">{{Consulta.modalidad}}</h3>
            ...
            <p class="text-documento costo-documento">Costo de documento: <span>S/ 20.00</span></p>
            ...
            <p class="text-documento precio-total">Precio total <span>S/ {{Consulta.precio}}</span></p>
...
export default {
    ...
    methods:{
        tipoPago(){
            axios.post("/legal/digital/tipo/pago",{numeroConsulta:this.numeroConsulta,modoPago:this.modoPago})
            .then((result) => {
             if(this.modoPago === "tarjeta")  {
                window.location.href = "/legal/digital/tarjeta/"+this.codigo
             }else if(this.modoPago === "deposito"){
                window.location.href = "/legal/digital/deposito/"+this.codigo
             }
            }).catch((err) => {
...
<style scoped >
.body-miabogado .titulo-codigo-descuento {
    margin-bottom: 1.5rem !important;
}
.body-miabogado .titulo-consulta{
    margin: 1.5rem;
}
.precio-total{
    margin-bottom: 1rem;
}
.titulo-codigo-descuento{
     margin: 1rem;
}
